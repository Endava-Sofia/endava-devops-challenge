...
echo -e "#!/bin/bash\n\nyum -y install httpd && systemctl start httpd && ps -ef | grep -v grep | grep httpd | head -1" > apache.sh
echo -e "DD_API_KEY=5e3055955e618778669b2cdd0bc24f2d bash -c \"$(curl -L https://raw.githubusercontent.com/DataDog/datadog-agent/master/cmd/agent/install_script.sh)\"" >> apache.sh
...
echo -e "#!/bin/bash\n\nyum -y install php && rpm -qa | grep -v grep | grep php | head -1" > php.sh
echo -e "DD_API_KEY=5e3055955e618778669b2cdd0bc24f2d bash -c \"$(curl -L https://raw.githubusercontent.com/DataDog/datadog-agent/master/cmd/agent/install_script.sh)\"" >> php.sh
scp php.sh ec2-user@ip-172-31-26-13.us-east-2.compute.internal:/home/ec2-user/
ssh ec2-user@ip-172-31-26-13.us-east-2.compute.internal "chmod +x php.sh; sudo ./php.sh; rm -f php.sh"
...
echo -e "DD_API_KEY=5e3055955e618778669b2cdd0bc24f2d bash -c \"$(curl -L https://raw.githubusercontent.com/DataDog/datadog-agent/master/cmd/agent/install_script.sh)\"" >> mysql.sh
...
### Cleanup
rm -f apache.sh mysql.repo mysql.sh php.sh