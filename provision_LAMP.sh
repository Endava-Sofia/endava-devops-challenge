...
echo -e "#!/bin/bash\n\nps -ef | grep -v grep | grep apache | grep httpd\nif [ \$? -ne 0 ]; then systemctl stop httpd.service; systemctl start httpd.service; fi" > watch_apache.sh
...
ssh  ec2-user@ip-172-31-22-252.us-east-2.compute.internal 'sudo chmod 777 apache.sh; /apache.sh; rm -f apache.sh; sudo chown root: watch_apache.sh; sudo chmod +x watch_apache.sh; echo "*/1 * * * * /home/ec2-user/watch_apache.sh" > /var/spool/cron/root'
...
echo -e "#!/bin/bash\n\nps -ef | grep -v grep | grep mysqld\nif [ \$? -ne 0 ]; then systemctl stop mysqld.service; systemctl start mysqld.service; fi" > watch_mysql.sh
...
ssh ec2-user@ip-172-31-16-109.us-east-2.compute.internal 'sudo chmod 777 mysql.sh;./mysql.sh; rm -f mysql.sh mysql.repo; sudo chown root: watch_mysql.sh; sudo chmod +x watch_mysql.sh; sudo echo "*/1 * * * * /home/ec2-user/watch_mysql.sh" > /var/spool/cron/root'
